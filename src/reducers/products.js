import {
  ...
  SUCCESS_LOADING_PRODUCTS, ERROR_LOADING_PRODUCTS,
  SUCCESS_LOADING_PRODUCT_CATEGORIES,} from "../actions"
...
export default function productsReducer(state = defaultState, action) {
  ...
  switch (action.type) {
    ...
    case SUCCESS_LOADING_PRODUCT_CATEGORIES:
      const foundProduct = state.list.find(product => product.id == action.payload.product.id)

      if (foundProduct) {
        return state 
        }else{
        return {
          ...state, list: state.list.concat(action.payload.product),
        }
      };
    default:
      return state;
